# Mermaid Diagram: LLM → Deterministic Graph Generation

## Problem
The LLM was inventing and reversing edges in the mermaid dependency diagram, even when explicitly told not to. The diagram was unreliable.

## Solution
Moved mermaid diagram generation out of the LLM entirely. The diagram is now generated deterministically from the real `madge` dependency graph.

## Changes Made

### [server.ts](file:///d:/Cp/proj/Codebase%20Explainer/src/server.ts)
...
| Change | Details |
|--------|---------|
| **New function** `generateMermaidFromGraph()` | Converts `Record<string, string[]>` → valid Mermaid `graph TD` syntax. Caps at 15 nodes by picking highest in-degree + out-degree. |
| **LLM prompt** simplified | Removed `mermaidDiagram` from the JSON schema, removed the "≤12 nodes" instruction and mermaid example. LLM now only returns `overview`, `entryPoints`, `readingOrder`. |
| **`/analyze` route** | Calls `generateMermaidFromGraph(dependencyGraph)` directly. Also returns `dependencyGraph` in the response for transparency. |
| **`generateMockAnalysis()`** | No longer produces its own mermaid — mock mode uses the same `generateMermaidFromGraph()` path as real mode (called from the route). |

## Verification: chalk/chalk Test

Ran `madge` on `chalk/chalk`'s source directory and compared the raw graph to the generated mermaid.
...
```json
{
  "index.js": [
    "utilities.js",
    "vendor/ansi-styles/index.js",
    "vendor/supports-color/index.js"
  ],
  "utilities.js": [],
  "vendor/ansi-styles/index.js": [],
  "vendor/supports-color/browser.js": [],
  "vendor/supports-color/index.js": []
}
```
...
graph TD
    N0["index.js"]
    N1["utilities.js"]
    N2["vendor/ansi-styles/index.js"]
    N3["vendor/supports-color/browser.js"]
    N4["vendor/supports-color/index.js"]
    N0 --> N1
    N0 --> N2
    N0 --> N4
```

**Result:** ✓ ALL mermaid edges match the raw dependency graph EXACTLY — no invented, reversed, or missing edges.

> [!TIP]
> TypeScript compiles clean with `tsc --noEmit --project tsconfig.json` — zero errors.
